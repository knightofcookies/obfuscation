...
from obfuscation.types import BenchmarkMode, BenchmarkResolution
...
def render() -> None:
	...
	BENCHMARK_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = obfuscation.choices.benchmark_modes,
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = obfuscation.choices.benchmark_resolutions,
	...
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(obfuscation.choices.benchmark_cycle_count_range),
		minimum = obfuscation.choices.benchmark_cycle_count_range[0],
		maximum = obfuscation.choices.benchmark_cycle_count_range[-1]
	)
