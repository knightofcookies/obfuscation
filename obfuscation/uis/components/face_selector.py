...
from obfuscation.face_analyser import get_many_faces
from obfuscation.face_selector import sort_and_filter_faces
from obfuscation.face_store import clear_static_faces
from obfuscation.filesystem import is_image, is_video
from obfuscation.types import FaceSelectorMode, FaceSelectorOrder, Gender, Race, VisionFrame
from obfuscation.uis.core import get_ui_component, get_ui_components, register_ui_component
from obfuscation.uis.types import ComponentOptions
from obfuscation.uis.ui_helper import convert_str_none
from obfuscation.vision import fit_cover_frame, read_static_image, read_video_frame
...
def render() -> None:
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = obfuscation.choices.face_selector_modes,
	...
	with gradio.Group():
		with gradio.Row():
			FACE_SELECTOR_ORDER_DROPDOWN = gradio.Dropdown(
				...
				choices = obfuscation.choices.face_selector_orders,
			...
			FACE_SELECTOR_GENDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + obfuscation.choices.face_selector_genders,
			...
			FACE_SELECTOR_RACE_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + obfuscation.choices.face_selector_races,
				value = state_manager.get_item('face_selector_race') or 'none'
			)
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or obfuscation.choices.face_selector_age_range[0]
			face_selector_age_end = state_manager.get_item('face_selector_age_end') or obfuscation.choices.face_selector_age_range[-1]
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				label = translator.get('uis.face_selector_age_range_slider'),
				minimum = obfuscation.choices.face_selector_age_range[0],
				maximum = obfuscation.choices.face_selector_age_range[-1],
				value = (face_selector_age_start, face_selector_age_end),
				step = calculate_int_step(obfuscation.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = translator.get('uis.reference_face_distance_slider'),
		value = state_manager.get_item('reference_face_distance'),
		step = calculate_float_step(obfuscation.choices.reference_face_distance_range),
		minimum = obfuscation.choices.reference_face_distance_range[0],
		maximum = obfuscation.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
