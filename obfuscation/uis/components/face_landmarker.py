...
from obfuscation import face_landmarker, state_manager, translator
from obfuscation.common_helper import calculate_float_step
from obfuscation.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = obfuscation.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(obfuscation.choices.face_landmarker_score_range),
		minimum = obfuscation.choices.face_landmarker_score_range[0],
		maximum = obfuscation.choices.face_landmarker_score_range[-1]
	)
