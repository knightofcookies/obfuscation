...
import obfuscation.jobs.job_manager
import obfuscation.jobs.job_store
from obfuscation import config, content_analyser, face_classifier, face_detector, face_landmarker, face_masker, face_recognizer, inference_manager, logger, state_manager, translator, video_manager
from obfuscation.common_helper import get_first, is_macos
from obfuscation.download import conditional_download_hashes, conditional_download_sources, resolve_download_url
from obfuscation.execution import has_execution_provider
from obfuscation.face_analyser import get_average_face, get_many_faces, get_one_face, scale_face
from obfuscation.face_helper import paste_back, warp_face_by_face_landmark_5
from obfuscation.face_masker import create_area_mask, create_box_mask, create_occlusion_mask, create_region_mask
from obfuscation.face_selector import select_faces, sort_faces_by_order
from obfuscation.filesystem import filter_image_paths, has_image, in_directory, is_image, is_video, resolve_relative_path, same_file_extension
from obfuscation.model_helper import get_static_model_initializer
from obfuscation.processors.modules.face_swapper import choices as face_swapper_choices
from obfuscation.processors.modules.face_swapper.types import FaceSwapperInputs
from obfuscation.processors.pixel_boost import explode_pixel_boost, implode_pixel_boost
from obfuscation.processors.types import ProcessorOutputs
from obfuscation.program_helper import find_argument_group
from obfuscation.thread_helper import conditional_thread_semaphore
from obfuscation.types import ApplyStateItem, Args, DownloadScope, Embedding, Face, InferencePool, ModelOptions, ModelSet, ProcessMode, VisionFrame
from obfuscation.vision import read_static_image, read_static_images, read_static_video_frame, unpack_resolution
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		obfuscation.jobs.job_store.register_step_keys([ 'face_swapper_model', 'face_swapper_pixel_boost', 'face_swapper_weight' ])
...
def forward_swap_face(source_face : Face, target_face : Face, crop_vision_frame : VisionFrame) -> VisionFrame:
	...
	if is_macos() and has_execution_provider('coreml') and model_type in [ 'ghost', 'uniface' ]:
		face_swapper.set_providers([ obfuscation.choices.execution_provider_set.get('cpu') ])
